...
public class BitPacker32<T> {
	...
	protected void addElement(BitElement element) {
		...
		this.bitMask = BitKit.intBitMask(totalBitLength);

		if (totalBitLength > 32) {
			throw new IndexOutOfBoundsException(String.format("BitPacker length %d exceeds limit of 32", totalBitLength));
		}
	}
	...
	protected abstract class BitElement {
		...
		private BitElement(int valueCount) {
			...
			this.mask = BitKit.intBitMask(this.bitLength);
		}
